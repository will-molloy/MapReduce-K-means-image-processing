...
sealed case class PointColorF(r: Double, g: Double, b: Double) {
  ...
  def ??(that: Seq[PointColorF]): PointColorF = that.reduceLeft((a, b) => if ((this >< a) < (this >< b)) a else b)

  def ><(that: PointColorF): Double = !(this - that)

  def -(that: PointColorF) = PointColorF(r - that.r, g - that.g, b - that.b)

  def unary_!(): Double = math.sqrt(r * r + b * b + g * g)
    ...
    for (a <- 0 until h * w) {
      ...
      near match {
        case PointColorF(hh, ss, vv) =>
          val pixel = Color.HSBtoRGB(hh.asInstanceOf[Float], ss.asInstanceOf[Float], vv.asInstanceOf[Float])
          img.setRGB(a / h, a % h, pixel)
      }
    }
